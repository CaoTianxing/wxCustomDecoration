...
Page({
  ...
  oTouchStart: function(e) {
    ...
    for (let i = 0; i < items.length; i++) {
      items[i].active = false;
      if (e.currentTarget.dataset.id == items[i].id) {
        console.log('点击获取当前点击的图片', 'e.currentTarget.dataset.id', e.currentTarget.dataset.id)
        index = i;
        items[index].active = true;
      }
    }
  ...
  oTouchMove: function(e) {
    ...
    items[index].ty = e.touches[0].clientY;
    items[index].anglePre = this.countDeg(items[index].x, items[index].y, items[index].tx, items[index].ty)
    console.log('移动后重新记录新的item',items)
    //赋值setData渲染
    this.setData({
      itemList: items
    })
  ...
  addzs() {
    this.setDropItem({
      url: 'https://zqcr.duodianwangluo.com/uploads/20190525/0eec358dd7fba7fcada137a5bc63f769.png'
    });
  },
  bs(){
    for(var i = 0; i<this.data.itemList.length;i++){
      this.data.itemList[i].active = false
    }
    wx.setStorage({
      key: "ctx",
